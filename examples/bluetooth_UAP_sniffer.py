...
class my_graph(gr.flow_graph):
	...
	if options.lap is None:
	    parser.print_help()
            sys.stderr.write('You must specify the LAP with -l LAP\n');
            raise SystemExit, 1

	#Set up the packet sniffer
	LAP = int(options.lap, 16)
	pkts = int(options.packets, 10)
	self.dst = bluetooth.UAP(LAP, pkts)
